...
class Game_Logic :
    ...
    def __init__(self,player_1,pc_1):
        ...
        #variable de que elige random el objeto jugador que empieza, de forma aleatoria 
        self.starter_turn = random.choice([player_1,pc_1])

    def print_boards(self):                  # aqui imprime el barco en el tablero, depende si quien sea el turno
    ...
    #Crea los barcos tanto en jugador maquina como en humano
    ...
    def game_running(self):
        ...
        while True:
            ...
            if isinstance(self.turns[i],Player):            # aqui empezamos nosotros para jugar, o por el contrario, pasa la compu
                #si es instancia de PLayer print el nombre del jugador en turno
                print(f"----> {self.turns[i].name} TURN <----")
                
                user_select = selection_ok() # imprime la seleccion 1,2,3 (user intrface)
                if user_select == 1:
                    self.print_boards()
